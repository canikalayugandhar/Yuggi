...
def run_ymc_scan_on_candles(candles, min_candles_required: Optional[int] = None, sl_pct: float = 0.1, tp_pct: float = 0.1, allow_intrabar: bool = False, live_ltp: Optional[float] = None):
    ...
    while True:
        ...
        # 🎯 LIVE SIGNAL: Start as PENDING, will be monitored in real-time for outcome
        # All new signals start as PENDING and get updated based on live price movements
...
def _within_market_hours(now_dt: dt.datetime) -> bool:
    ...
    if now_dt.tzinfo is None:
        local = now_dt.replace(tzinfo=IST)
    else:
    ...
    return is_weekday and market_start <= t <= market_end
