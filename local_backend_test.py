#!/usr/bin/env python3
"""
Local Trinity Wealth Scanner Backend Testing
Focus: WIN/LOSS Calculation Fix Validation - Testing localhost
"""
...
# Test Configuration - Use localhost
...
def test_scanner_status():
    ...
    try:
        response = requests.get(f"{BACKEND_URL}/scanner/status", timeout=5)
        if response.status_code == 200:
            data = response.json()
            is_running = data.get('is_running', False)
            error_msg = data.get('error_message', 'None')
            stats = data.get('stats', {})
            logger.info(f"✅ Scanner status: Running={is_running}")
...
def test_signals_endpoint():
    ...
    try:
        response = requests.get(f"{BACKEND_URL}/scanner/signals", timeout=5)
        if response.status_code == 200:
            signals = response.json()
            logger.info(f"✅ Signals endpoint: Retrieved {len(signals)} signals")
            
            if len(signals) == 0:
                logger.info("   No signals found - this is expected if scanner is not generating signals")
                return True
...
def test_reset_outcomes_endpoint():
    ...
    try:
        response = requests.post(f"{BACKEND_URL}/scanner/reset-outcomes", timeout=5)
        if response.status_code == 200:
            data = response.json()
...
def main():
    ...
    logger.info("🚀 Trinity Wealth Scanner - WIN/LOSS Calculation Fix Testing (Local)")
    logger.info("="*70)
